...
# 使用方法: gitcp "<提交信息>"
# 示例:
#   gitcp "fix: 修复登录问题"           # 提交并推送
#   gitcp "feat: 添加用户管理功能"      # 功能提交
#   gitcp "docs: 更新 README"          # 文档更新
gitcp() {
    if [[ -z "$1" ]]; then
        echo "请提供提交信息"
        return 1
...
# 示例:
#   gitco                     # 显示所有分支
#   gitco main                # 切换到 main 分支
#   gitco feature/user-auth   # 切换到功能分支
#   gitco -b new-feature      # 创建并切换到新分支
gitco() {
    if [[ -z "$1" ]]; then
        git branch -a
        return
...
# 使用方法: checkport <端口号>
# 示例:
#   checkport 3000               # 检查3000端口占用情况
#   checkport 8080               # 检查8080端口
#   checkport 22                 # 检查SSH端口
checkport() {
    if [[ -z "$1" ]]; then
        echo "使用方法: checkport <端口号>"
        return 1
    fi
    lsof -i :"$1"
}
...
# 示例:
#   getweather                 # 查询当前位置天气
#   getweather "Beijing"       # 查询北京天气
#   getweather "New York"      # 查询纽约天气
#   getweather "Shanghai"      # 查询上海天气
getweather() {
    local city=${1:-}
