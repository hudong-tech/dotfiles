...
在开始配置之前，了解conda的核心目录结构：

#### **~/.conda/envs 目录**

这是conda的**环境存储目录**，主要用途：

- **存储虚拟环境**：每个创建的conda环境都会在这里有一个对应的目录
- **完整文件结构**：包含环境的Python解释器、包和配置文件
- **环境管理**：创建、激活、删除环境时的核心操作目录

#### **~/.conda/pkgs 目录**

这是conda的**包缓存目录**，主要用途：

- **包存储**：存储所有下载的conda包，包括元数据和实际文件
- **性能优化**：避免重复下载相同的包，加速环境创建和更新
- **空间管理**：随时间增长，需要定期清理以节省磁盘空间

#### **目录维护建议**

```bash
# 查看目录大小
du -sh ~/.conda/envs ~/.conda/pkgs
